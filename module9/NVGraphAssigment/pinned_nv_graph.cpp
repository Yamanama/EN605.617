...
void allocate(Lists* lists, size_t n, size_t nnz, size_t vertex, size_t edge) {
    cudaMallocHost((void**)&lists->destination_offsets_h, (n+1)*sizeof(int));
    cudaMallocHost((void**)&lists->source_indices_h, nnz*sizeof(int));
    cudaMallocHost((void**)&lists->weights_h, (nnz)*sizeof(float));
    cudaMallocHost((void**)&lists->bookmark_h, n*sizeof(float));
    cudaMallocHost((void**)&lists->pr_1, n*sizeof(float));
...
void cleanUp(nvgraphHandle_t handle, nvgraphGraphDescr_t graph, Lists lists) {
    ...
    cudaFreeHost(lists.destination_offsets_h);
    cudaFreeHost(lists.source_indices_h);
    cudaFreeHost(lists.weights_h);
    cudaFreeHost(lists.bookmark_h);
    cudaFreeHost(lists.pr_1);
}
/**
 * Main
 */

                      ...
                      const char* message) 
{
    ...
    cudaMemcpyAsync(results->sequence,
               device.block,
               options.arraySize,
                    ...
                    const char* message) 
{
    ...
    cudaEventRecord(start, 0);
    // store generated sequences
    cudaMemcpyAsync(results->random,
        device.block,
        options.arraySize,
        cudaMemcpyDeviceToHost,
        stream);
...
/**
 * Stream the sequence and random into the device
 * 
 * results - results structure
 * device - device structure
 * options - cli structure
 */
void streamToDevice(Results* results, Device device, Options options) {
    // stream
    cudaStream_t stream;
    cudaStreamCreate(&stream);
    // markers
    cudaEvent_t start, stop;
    printf("  Streaming to device\n");
    // set up timer
    setupTimer(&start, &stop);
    // mark time
    cudaEventRecord(start, 0);
    // store generated sequences
    cudaMemcpyAsync(device.sequence, 
        results->sequence, 
        options.arraySize,
        cudaMemcpyDeviceToHost,
        stream);
    cudaMemcpyAsync(device.random, 
        results->random, 
        options.arraySize,
        cudaMemcpyDeviceToHost,
        stream);       
    ...
    cudaStreamSynchronize(stream);
    // log time
    logTime(start, stop, STREAM_STRING);
}
/**
 * Perform addition
 *
...
void performSubtract(Results* results, Device device, Options options)
{
    ...
    cudaEventRecord(start, 0);
    // store generated sequences
    cudaMemcpyAsync(results->difference, 
               device.block, 
               options.arraySize,
               cudaMemcpyDeviceToHost,
               stream);
...
void performModulus(Results* results, Device device, Options options)
{
    ...
    cudaEventRecord(start, 0);
    // store generated sequences
    cudaMemcpyAsync(results->modulus, 
               device.block, 
               options.arraySize,
               cudaMemcpyDeviceToHost,
               stream);
...
void performMult(Results* results, Device device, Options options)
{
    ...
    cudaEventRecord(start, 0);
    // store generated sequences
    cudaMemcpyAsync(results->product, 
               device.block, 
               options.arraySize,
               cudaMemcpyDeviceToHost,
               stream);
             ...
             const char * message)
{
    ...
    // stream
    streamToDevice(&results, device, options);
    // perform
